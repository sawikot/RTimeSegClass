...
def unet_block(n_depth=2, n_filter_base=16, kernel_size=(3,3), n_conv_per_depth=2,
    ...
    def _func(input):
        ...
        #---------------ResNet50---------------
        model = tf.keras.applications.resnet50.ResNet50(input_tensor=layer, include_top=False,weights=None)
        model.summary()

        skip_layers.append(model.get_layer('conv1_bn').output)
        skip_layers.append(model.get_layer('conv1_bn').output)
        skip_layers.append(model.get_layer('conv2_block3_add').output)
        layer=model.get_layer('conv3_block4_out').output
...
class StarDist2D(StarDistBase):
    ...
    def _build(self):
        ...
        plot_model(finl_model, to_file='ResNet50_UNet.png', show_shapes=True, show_layer_names=True)
