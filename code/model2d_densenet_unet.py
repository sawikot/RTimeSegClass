...
def unet_block(n_depth=2, n_filter_base=16, kernel_size=(3,3), n_conv_per_depth=2,
    ...
    def _func(input):
        ...
        #---------------densenet169---------------
        model = tf.keras.applications.densenet.DenseNet169(input_tensor=layer, include_top=False,weights=None)
        #model.summary()

        
        skip_layers.append(model.get_layer('conv1/relu').output)
        skip_layers.append(model.get_layer('conv1/relu').output)
        skip_layers.append(model.get_layer('pool2_conv').output)
        #skip_layers.append(model.get_layer('pool3_conv').output)
        layer=model.get_layer('pool3_conv').output
        #skip_layers.append(model.get_layer('conv5_block32_concat').output)
...
class StarDist2D(StarDistBase):
    ...
    def _build(self):
        ...
        plot_model(finl_model, to_file='VGG_UNet.png', show_shapes=True, show_layer_names=True)
