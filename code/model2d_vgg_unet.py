...
def unet_block(n_depth=2, n_filter_base=16, kernel_size=(3,3), n_conv_per_depth=2,
    ...
    def _func(input):
        ...
        #---------------VGG16---------------
        model = tf.keras.applications.VGG16(input_tensor=layer, include_top=False,weights=None)
        #model.summary()

        skip_layers.append(model.get_layer('block1_conv2').output)
        skip_layers.append(model.get_layer('block2_conv2').output)
        skip_layers.append(model.get_layer('block3_conv3').output)
        layer=model.get_layer('block3_pool').output
        #layer=model.get_layer('block5_pool').output
